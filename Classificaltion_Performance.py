...
def PT(fpr, sensitivity):
    ...
    pt = (np.sqrt((sensitivity / 100) * (fpr / 100)) - (fpr / 100)) / ((sensitivity / 100) - (fpr / 100))
...
def FM(sensitivity, precision):
    ...
    fm = np.sqrt((precision / 100) * (sensitivity / 100))
    return fm * 100
...
def Test(size=(1000, 1)):
    actual = np.random.randint(low=0, high=2, size=size)
    pred = np.random.randint(low=0, high=2, size=size)
